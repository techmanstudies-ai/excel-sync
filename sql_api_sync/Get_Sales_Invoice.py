...
WORKSHEET_NAME = "Sales_Invoice"

# Save progress/log in same folder as this Python file
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
PROGRESS_FILE = os.path.join(BASE_DIR, f"progress_{DOCUMENT_TYPE}_{TARGET_YEAR}.json")
...
def safe_request(url):
    for i in range(MAX_RETRY):
        try:
            ...
            print(f"API status {response.status_code}, retry {i + 1}/{MAX_RETRY}")

        except Exception as e:
            print(f"Request error: {e}, retry {i + 1}/{MAX_RETRY}")
...
def find_year_start():
    ...
    while True:
        ...
        if not response:
            print(f"Cannot fetch offset {offset}. Using last valid offset {last_valid_offset}.")
...
print("\nPhase 2: Downloading sales invoices...")
...
while True:
    ...
    save_progress(offset, total_pushed + len(batch_rows))
